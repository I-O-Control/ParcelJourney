const demoHeader=document.querySelector('header');const demoSubtitle=demoHeader?.querySelector('p');if(demoSubtitle)demoSubtitle.textContent='Real log replay demo';const demoNotice=document.querySelector('.notice');if(demoNotice)demoNotice.textContent='Real log replay demo · Event times are recorded; positions between observed equipment are schematic.';const headerActions=demoHeader?.querySelector('.header-right');if(headerActions){let view3d=headerActions.querySelector('.view3d-link')||headerActions.querySelector('a[href^="/3d"]');if(!view3d){view3d=document.createElement('a');view3d.className='view3d-link';headerActions.append(view3d)}view3d.href='/3d';view3d.textContent='Explore 3D ↗';view3d.style.cssText='color:var(--muted);font-size:11px;text-decoration:none;white-space:nowrap'}
